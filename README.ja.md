...
RAG や AI Agent では、外部から取得した文書がそのまま LLM のコンテキストに注入されます。
攻撃者がその文書に悪意ある命令を仕込んでいれば、LLM は気づかずに実行してしまいます。
これが **Indirect Prompt Injection（間接プロンプトインジェクション）** です。

ContextGate は文書が LLM に届く前にスキャンし、危険なコンテンツを未然にブロックします。
...
ContextGate は現状有姿（"as is"）で提供され、明示・黙示を問わず、いかなる保証も行いません。
本ソフトウェアの使用または使用不能に起因するセキュリティインシデント、データ漏洩、システム障害その他の損害について、
作者および貢献者は一切の責任を負いません。

ContextGate はすべてのプロンプトインジェクション攻撃を検知することを保証しません。
本ツールは多層防御の一要素として活用することを意図しており、システムの単独のセキュリティ対策として使用しないでください。
